            ...
            steps {
                script {
                    dir(WORKSPACE_DIR) {
                        ...
                        sh "git fetch origin"
                        def branchExists = sh(script: "git branch -r | grep -q 'origin/${env.CHANGE_TARGET}' && echo 'yes' || echo 'no'", returnStdout: true).trim()

                        if (isPR && branchExists == 'yes') {
                            ...
                            echo "Target branch does not exist: origin/${env.CHANGE_TARGET}, skipping diff check."
                        }
